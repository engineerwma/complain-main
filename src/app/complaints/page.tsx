...
export default function ComplaintsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setPagination(prev => ({
          ...prev,
          totalItems: safeComplaints.length,
          totalPages: Math.ceil(safeComplaints.length / prev.pageSize)
        }))

        // Ensure other data is arrays
        setFilterOptions({
          statuses: Array.isArray(statusesData) ? statusesData : [],
          types: Array.isArray(typesData) ? typesData : [],
          branches: Array.isArray(branchesData) ? branchesData : [],
          linesOfBusiness: Array.isArray(lobData) ? lobData : [],
          users: Array.isArray(usersData) 
            ? usersData.filter((user: any) => user.id && user.id.trim() !== '')
            : []
        })
  ...
  useEffect(() => {
    let result = [...complaints] // Create a copy to ensure it's an array
  ...
  useEffect(() => {
    // Ensure filteredComplaints is an array before slicing
    const safeFilteredComplaints = Array.isArray(filteredComplaints) ? filteredComplaints : []
    const startIndex = (pagination.currentPage - 1) * pagination.pageSize
    const endIndex = startIndex + pagination.pageSize
    setDisplayedComplaints(safeFilteredComplaints.slice(startIndex, endIndex))
  ...
  const exportToCSV = () => {
    ...
    if (safeFilteredComplaints.length === 0) {
    ...
    const csvContent = [
      ...
      '\uFEFF' + headers.join(","),
      ...safeFilteredComplaints.map(complaint => [
